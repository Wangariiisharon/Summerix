...
export default function DepartmentsTble({ departments, count }: DepartmentTableProps) {
  ...
  console.log('Deprtments', departments);
  ...
  const handleCheckboxChange = (department: DEPARTMENT) => {
    ...
    if (isDepartmentSelected) {
      setSelectedDepartments(selectedDepartments.filter((a) => a.docId !== department.docId));
    } else {
  ...
  const deleteUser = async (docId: any) => {
    try {
      await deleteDoc(doc(fbDb, 'departments', docId));
    } catch (error) {
  ...
  const toggleArchiveStatus = async (department: DEPARTMENT) => {
    try {
      const departmentRef = doc(fbDb, 'departments', department.docId as string);
  ...
  const deleteSelectedDepartments = async () => {
    try {
      for (const department of selectedDepartments) {
        if (department.docId) {
          await deleteDoc(doc(fbDb, 'departments', department.docId));
        } else {
          console.warn('Admin ID is undefined, skipping deletion for:', department);
        }
      }
    ...
    <div>
      ...
      <div className="overflow-x-auto rounded-lg bg-gray-100 shadow-md">
        ...
        <div className="table-wrapper">
                  ...
                  return (
                    <tr key={department.docId} className="tr-body">
                      ...
                      <td className="px-6 py-3">
                        <span className="">
                          {department.lastUpdated &&
                            format(department.lastUpdated.toDate(), 'MMMM d, yyyy h:mm a')}
                        </span>
                      </td>
          ...
          {Object.keys(selectedDepartments).filter((key) => selectedDepartments[Number(key)])
            .length > 0 && (
            <div className="mt-4 flex items-center justify-between rounded-lg bg-gray-100 p-4 shadow-md">
              <div className="flex flex-row">
                <span>
                  {
                ...
                <p
                  className="text-decoration-line: mr-2 rounded px-4 text-red-500 underline"
