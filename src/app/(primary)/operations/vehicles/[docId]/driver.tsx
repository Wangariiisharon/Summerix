...
export default function DriverAllocation({ vehicle }: Props) {
  const [selected, setSelected] = useState<DRIVER_DETAILS | null>(vehicle.driver);
  ...
  const doConfirmAllocation = async () => {
    ...
    try {
      ...
      const driverDocRef = doc(fbDb, Constants.fbDrivers, selected.docId);
      ...
      const vehicleDocRef = doc(fbDb, Constants.fbVehicles, vehicle.docId);
              ...
              <div className="relative mt-2 w-1/2">
                <ListboxButton className="relative w-full cursor-default rounded-md bg-white py-1.5 pl-3 pr-10 text-left text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 focus:outline-none focus:ring-2 focus:ring-primary sm:text-sm/6">
                  <div className="flex items-center gap-2">
                    ...
                    <p className="block truncate">{selected?.displayName}</p>
                  </div>
