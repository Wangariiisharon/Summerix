...
export default async function handler(
  ...
  if (!origins || !destinations) {
    return res.status(400).json({
      error: "Missing required query parameters: origins, destinations",
    });
  }
  ...
  const url = `https://maps.googleapis.com/maps/api/distancematrix/json?units=metric&origins=${encodeURIComponent(
    origins as string
  )}&destinations=${encodeURIComponent(destinations as string)}&key=${apiKey}`;
  ...
  try {
    ...
    if (data.status === "OK") {
      ...
      return res
        .status(500)
        .json({ error: "Failed to fetch distance", details: data });
    }
  } catch (error) {
    console.error("Error fetching distance:", error);
    return res
      .status(500)
      .json({ error: "Failed to fetch distance", details: error });
  }
}
