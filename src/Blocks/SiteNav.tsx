...
import { getFirestore, collection, doc, setDoc, addDoc,getDocs, DocumentData, getDoc, where, query, onSnapshot, orderBy, Timestamp } from 'firebase/firestore'; 
...
export default function SiteNav({ children }: Props) {
  ...
  useEffect(() => {
   ...
   const fetchNotifications = () => {
    ...
    const q = query(notificationsRef, where('organisationId', '==', organisationId));
    ...
    return onSnapshot(q, (snapshot) => {
      ...
      const unreadNotifications = loadedNotifications.filter(notification => 
        notification.readBy && !notification.readBy.includes(currentUser.uid)
      );
  ...
  return (
    <>
        ...
        <div className="flex flex-row">
          ...
          <div className={`flex flex-row fixed top-0 h-10 bg-[#FFFFFF] flex items-center shadow-inner ${isDrawerOpen ? 'lg:left-72 w-full' : 'w-full lg:w-auto'}`}>
       ...
       <div className="fixed right-14 w-8">
        <button onClick={toggleNotificationDropdown} className="text-gray-500 w-8 h-8 focus:outline-none relative">
        ...
        {unreadCount}
          ...
          <NotificationDropdown isOpen={showDropdown} onClose={closeDropdown}/>
        </div>
          ...
          <div
            className={`bg-[#FAFAFB] px-4 py-10 h-screen ${isDrawerOpen ? 'lg:w-full sm:px-6 lg:px-8' : 'lg:w-full '} transition-all ease-in-out duration-300`}
          >
            {children}
          </div> 
        </div>
